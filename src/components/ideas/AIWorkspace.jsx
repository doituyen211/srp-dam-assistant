...
import { parseAIResponse, generateSampleResponse } from "./parseAIResponse";
...
/**
 * AIWorkspace - Center column with prompt input and AI response cards
 */
export function AIWorkspace({ messages, onSend, onCardAction, isReadOnly }) {
  ...
  const renderCard = (msg, msgIndex) => {
          ...
          {msg.cards.map((card, cardIndex) => {
            switch (card.type) {
              case "research_direction":
                return <ResearchDirectionCard key={cardIndex} card={card} onAction={onCardAction} />;
              case "research_gap":
                return <ResearchGapCard key={cardIndex} card={card} onAction={onCardAction} />;
              case "methodology":
                return <MethodologyCard key={cardIndex} card={card} onAction={onCardAction} />;
              case "problem":
                return <ProblemCard key={cardIndex} card={card} onAction={onCardAction} />;
              default:
                return (
                  <div key={cardIndex} className="rounded-lg border border-hairline bg-canvas p-4">
                    <p className="text-sm text-ink">{card.content}</p>
                  </div>
                );
            }
          })}
    ...
    return (
      <div key={msgIndex} className="flex justify-start">
        <div className="max-w-[85%] rounded-2xl rounded-bl-md bg-subdued px-4 py-2.5 text-sm text-ink">
          {msg.content}
        </div>
      </div>
    ...
    <div className="flex h-full min-h-0 flex-col bg-canvas">
      ...
      <div className="flex items-center justify-between border-b border-hairline px-5 py-3">
        <div>
          ...
          <p className="text-[11px] text-muted">Describe your research idea — AI will suggest directions</p>
        </div>
      </div>
        ...
        {messages.length === 0 ? (
          <div className="flex h-full items-center justify-center">
            <div className="text-center max-w-sm">
              ...
              <h3 className="text-base font-semibold text-ink">Start with an idea</h3>
              <p className="mt-1.5 text-sm text-muted">Describe your research idea and AI will help you explore directions, gaps, and methodologies.</p>
            </div>
          </div>
        ) : (
          <div className="space-y-4">
            {messages.map((msg, i) => renderCard(msg, i))}
          </div>
        )}
      </div>
