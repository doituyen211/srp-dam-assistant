...
// Section key mapping: backend API keys → frontend SECTION_TYPES IDs
const SECTION_KEY_MAP = {
  research_problem: "problem",
  research_question: "question",
  literature_background: "literature",
  feasibility_timeline: "feasibility",
...
export function normalizeProposalSection(raw) {
  if (!raw) return null;
  const c = snakeToCamel(raw);
  const rawKey = c.key || c.id || c.section_id;
  const mappedKey = SECTION_KEY_MAP[rawKey] || rawKey;
  return {
    ...raw,
    ...c,
    id: mappedKey,
    section_id: c.section_id || rawKey,
    key: mappedKey,
    label: c.label || "",
    content: c.content || "",
    health: c.health || c.healthStatus || c.health_status || c.status || "missing",
    status: c.status || c.health || c.healthStatus || c.health_status || "missing",
...
export function normalizeAIReview(raw) {
  ...
  return {
    ...
    overallReadiness: c.overallReadiness ?? c.overall_readiness ?? c.score ?? c.readiness ?? 0,
    overall_readiness: c.overall_readiness ?? c.overallReadiness ?? c.score ?? c.readiness ?? 0,
    score: c.score ?? c.readiness ?? c.overallReadiness ?? c.overall_readiness ?? 0,
    readiness: c.readiness ?? c.score ?? c.overallReadiness ?? c.overall_readiness ?? 0,
...
export function normalizeRubricReview(raw) {
  ...
  return {
    ...
    proposalId: c.proposalId || c.proposal_id || null,
    proposal_id: c.proposal_id || c.proposalId || null,
    reviewer: c.reviewer || c.reviewerName || c.reviewer_name || "",
    reviewer_name: c.reviewer_name || c.reviewerName || c.reviewer || "",
...
export function normalizeReviewQueueItem(raw) {
  ...
  const rubric = raw?.latestRubricReview || raw?.latest_rubric_review
    ? normalizeRubricReview(raw.latestRubricReview || raw.latest_rubric_review)
    : null;
...
export function normalizeAdminOverview(raw) {
  if (!raw) return null;
  const c = snakeToCamel(raw);
  const stageCounts = c.proposalsByStage || c.proposals_by_stage || c.countsByStage || c.counts_by_stage || {};
  return {
    ...raw,
    ...c,
    ...
    pendingReviews: c.pendingReviews ?? c.pending_reviews ?? c.awaitingReview ?? c.awaiting_review ?? 0,
    pending_reviews: c.pending_reviews ?? c.pendingReviews ?? 0,
    awaitingReview: c.awaitingReview ?? c.awaiting_review ?? c.pendingReviews ?? c.pending_reviews ?? 0,
    awaiting_review: c.awaiting_review ?? c.awaitingReview ?? 0,
...
export function normalizeAuditLog(raw) {
  ...
  return {
    ...
    userId: c.userId || c.user_id || c.actorId || c.actor_id || null,
    user_id: c.user_id || c.userId || c.actor_id || c.actorId || null,
    actorId: c.actorId || c.actor_id || c.userId || c.user_id || null,
    actor_id: c.actor_id || c.actorId || c.user_id || c.userId || null,
    userName: c.userName || c.user_name || c.actorName || c.actor_name || "System",
    user_name: c.user_name || c.userName || c.actor_name || c.actorName || "System",
    actorName: c.actorName || c.actor_name || c.userName || c.user_name || "System",
    entityType: c.entityType || c.entity_type || null,
    entity_type: c.entity_type || c.entityType || null,
    entityId: c.entityId || c.entity_id || null,
    entity_id: c.entity_id || c.entityId || null,
  };
}

export function normalizeLecturer(raw) {
  if (!raw) return null;
  const c = snakeToCamel(raw);
  return {
    ...raw,
    ...
    currentLoad: c.currentLoad ?? c.current_load ?? 0,
    current_load: c.current_load ?? c.currentLoad ?? 0,
    maxLoad: c.maxLoad ?? c.max_load ?? 3,
    max_load: c.max_load ?? c.maxLoad ?? 3,
...
export function normalizeMilestone(raw) {
  ...
  return {
    ...
    due_date: c.due_date || c.dueDate || null,
...
export function normalizeFaculty(raw) {
  ...
  return {
    ...
    id: c.id,
    name: c.name || "",
    departmentCount: c.departmentCount ?? c.department_count ?? 0,
    department_count: c.department_count ?? c.departmentCount ?? 0,
...
export function normalizeDepartment(raw) {
  ...
  return {
    ...
    id: c.id,
    name: c.name || "",
    facultyId: c.facultyId || c.faculty_id || null,
    faculty_id: c.faculty_id || c.facultyId || null,
    facultyName: c.facultyName || c.faculty_name || null,
    faculty_name: c.faculty_name || c.facultyName || null,
    isActive: c.isActive ?? c.is_active ?? true,
    is_active: c.is_active ?? c.isActive ?? true,
  };
}

export function normalizeFieldOfStudy(raw) {
  if (!raw) return null;
  const c = snakeToCamel(raw);
  return {
    ...raw,
